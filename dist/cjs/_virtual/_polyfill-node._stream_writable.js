...
_polyfillNode__inherits(Writable, _polyfillNode_events.default);
...
function WritableState(options, stream) {
  ...
  if (stream instanceof _polyfillNode__stream_duplex.default) this.objectMode = this.objectMode || !!options.writableObjectMode;
...
function Writable(options) {
  ...
  if (!(this instanceof Writable) && !(this instanceof _polyfillNode__stream_duplex.default)) return new Writable(options);
...
function writeAfterEnd(stream, cb) {
  ...
  _polyfillNode_process.nextTick(cb, er);
}
...
function onwriteError(stream, state, sync, er, cb) {
  --state.pendingcb;
  if (sync) _polyfillNode_process.nextTick(cb, er);else cb(er);
...
function onwrite(stream, er) {
  ...
  if (er) onwriteError(stream, state, sync, er, cb);else {
    ...
    if (sync) {
      /*<replacement>*/
        _polyfillNode_process.nextTick(afterWrite, stream, state, finished, cb);
      /*</replacement>*/
    } else {
...
function endWritable(stream, state, cb) {
  ...
  finishMaybe(stream, state);
  if (cb) {
    if (state.finished) _polyfillNode_process.nextTick(cb);else stream.once('finish', cb);
  }
  state.ended = true;
