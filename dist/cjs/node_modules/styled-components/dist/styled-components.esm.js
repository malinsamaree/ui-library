        ...
        try {
            ...
            console.error = function (consoleErrorMessage) {
                ...
                if (invalidHookCallRe.test(consoleErrorMessage)) {
                    ...
                    originalConsoleError_1.apply(void 0, tslib_es6.__spreadArray([consoleErrorMessage], consoleErrorArgs, false));
                }
            };
            // We purposefully call `useRef` outside of a component and expect it to throw
            // If it doesn't, then we're inside another component.
            // eslint-disable-next-line react-hooks/rules-of-hooks
            index$1.react.exports.useRef();
...
function throwStyledComponentsError(code) {
    ...
    if (process.env.NODE_ENV === 'production') {
        ...
        return new Error(format.apply(void 0, tslib_es6.__spreadArray([ERRORS[code]], interpolations, false)).trim());
    }
}
...
var StyleSheet = /** @class */ (function () {
    function StyleSheet(options, globalStyles, names) {
        ...
        this.options = tslib_es6.__assign(tslib_es6.__assign({}, defaultOptions), options);
    ...
    StyleSheet.prototype.reconstructWithOptions = function (options, withNames) {
        ...
        return new StyleSheet(tslib_es6.__assign(tslib_es6.__assign({}, this.options), options), this.gs, (withNames && this.names) || undefined);
    };
...
function createStylisInstance(_a) {
    ...
    var selfReferenceReplacementPlugin = function (element) {
        if (element.type === Enum.RULESET && element.value.includes('&')) {
    ...
    var stringifyRules = function (css, selector, prefix, componentId) {
        ...
        if (options.prefix || options.prefix === undefined) {
            middlewares.unshift(Middleware.prefixer);
        }
        middlewares.push(selfReferenceReplacementPlugin, Serializer.stringify);
        return serialize(Parser.compile(prefix || selector ? "".concat(prefix, " ").concat(selector, " { ").concat(flatCSS, " }") : flatCSS), Middleware.middleware(middlewares));
    };
...
var StyleSheetContext = index.createContext(undefined);
StyleSheetContext.Consumer;
var StylisContext = index.createContext(undefined);
StylisContext.Consumer;
var mainSheet = new StyleSheet();
var mainStylis = createStylisInstance();
function useStyleSheet() {
    return index$1.react.exports.useContext(StyleSheetContext) || mainSheet;
}
function useStylis() {
    return index$1.react.exports.useContext(StylisContext) || mainStylis;
}
...
var objToCssArray = function (obj, prevKey) {
    ...
    return prevKey ? tslib_es6.__spreadArray(tslib_es6.__spreadArray(["".concat(prevKey, " {")], rules, true), ['}'], false) : rules;
};
...
var ThemeContext = index.createContext(undefined);
...
function useInjectedStyle(componentStyle, isStatic, resolvedAttrs, warnTooManyClasses) {
    ...
    // eslint-disable-next-line react-hooks/rules-of-hooks
    if (process.env.NODE_ENV !== 'production')
        index$1.react.exports.useDebugValue(className);
...
function useStyledComponentImpl(forwardedComponent, props, forwardedRef, isStatic) {
    ...
    // eslint-disable-next-line react-hooks/rules-of-hooks
    if (process.env.NODE_ENV !== 'production')
        index$1.react.exports.useDebugValue(styledComponentId);
    // NOTE: the non-hooks version only subscribes to this when !componentStyle.isStatic,
    // but that'd be against the rules-of-hooks. We could be naughty and do it anyway as it
    // should be an immutable value, but behave for now.
    var theme = determineTheme(props, index$1.react.exports.useContext(ThemeContext), defaultProps) || EMPTY_OBJECT;
        ...
        for (var key in resolvedAttrDef) {
            ...
            p[key] =
                key === 'className'
                    ...
                    : key === 'style'
                        ? tslib_es6.__assign(tslib_es6.__assign({}, p[key]), resolvedAttrDef[key]) : resolvedAttrDef[key];
        }
        /* eslint-enable guard-for-in */
        return p;
    }, tslib_es6.__assign(tslib_es6.__assign({}, props), { theme: theme }));
...
function createStyledComponent(target, options, rules) {
     ...
     */
    var WrappedStyledComponent = index.forwardRef(forwardRef);
    ...
    WrappedStyledComponent.withComponent = function withComponent(tag) {
        var previousComponentId = options.componentId, optionsToCopy = tslib_es6.__rest(options, ["componentId"]);
        var newComponentId = previousComponentId &&
            "".concat(previousComponentId, "-").concat(isTag(tag) ? tag : escape(getComponentName(tag)));
        var newOptions = tslib_es6.__assign(tslib_es6.__assign({}, optionsToCopy), { attrs: finalAttrs, componentId: newComponentId });
...
function css(styles) {
    ...
    if (isFunction(styles) || isPlainObject(styles)) {
        ...
        return addTag(flatten(interleave(EMPTY_ARRAY, tslib_es6.__spreadArray([
...
function constructWithOptions(componentConstructor, tag, options) {
    ...
    var templateFunction = function (initialStyles) {
        ...
        return componentConstructor(tag, options, css.apply(void 0, tslib_es6.__spreadArray([initialStyles], interpolations, false)));
    };
    /* Modify/inject new props at runtime */
    templateFunction.attrs = function (attrs) {
        return constructWithOptions(componentConstructor, tag, tslib_es6.__assign(tslib_es6.__assign({}, options), { attrs: Array.prototype.concat(options.attrs, attrs).filter(Boolean) }));
    };
    /**
     * If config methods are called, wrap up a new template function and merge options */
    templateFunction.withConfig = function (config) {
        return constructWithOptions(componentConstructor, tag, tslib_es6.__assign(tslib_es6.__assign({}, options), config));
    };
